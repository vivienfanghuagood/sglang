...
_NSA_IMPL_T: TypeAlias = Literal[
    "flashmla_prefill", "flashmla_decode", "fa3", "tilelang", "triton_sparse"
]
...
class NativeSparseAttnBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        elif NSA_PREFILL_IMPL == "triton_sparse":
            if q_rope is not None:
                q_all = torch.cat([q_nope, q_rope], dim=-1)
    ...
    def forward_decode(
        ...
        elif NSA_DECODE_IMPL == "triton_sparse":
            if q_rope is not None:
                q_all = torch.cat([q_nope, q_rope], dim=-1)
            return self._forward_triton_sparse(
                q_all=q_all,
    ...
    def _forward_triton_sparse(
        ...
        # triton_flash_mla_sparse_fwd expects 4D tensors with batch dimension:
        # q: [batch, s_q, h_q, d_qk]
        # kv: [batch, s_kv, h_kv, d_qk]
        # indices: [batch, s_q, h_kv, topk]
        #
        # Matching tilelang's approach: add batch dimension without memory copy
        # Triton uses strides so contiguous() is NOT needed

        # Add batch dimension (no memory copy - just view change)
        q_4d = q_all.unsqueeze(0)  # [1, s_q, h_q, d_qk]
        kv_4d = kv_cache.unsqueeze(0)  # [1, s_kv, h_kv, d_qk]
        indices_4d = page_table_1.unsqueeze(0).unsqueeze(2)  # [1, s_q, 1, topk]

        # Ensure indices are int32 (this may copy if dtype differs)
        if indices_4d.dtype != torch.int32:
            indices_4d = indices_4d.to(torch.int32)

        out, _, _ = triton_flash_mla_sparse_fwd(
            q=q_4d,
            kv=kv_4d,
            indices=indices_4d,
            sm_scale=sm_scale,
        ...
        # Remove batch dimension (no memory copy - just view change)
