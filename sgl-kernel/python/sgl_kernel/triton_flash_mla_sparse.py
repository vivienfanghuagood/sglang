...
def reference_torch_sparse_attention(
    ...
    """Reference PyTorch implementation with batch support.
    
    Process batch-by-batch to reduce memory usage for large inputs.
    """
    ...
    # Process batch-by-batch to reduce memory
    for b in range(batch):
        # indices: [batch, s_q, h_kv, topk] -> [s_q, topk] for this batch
        indices_b = indices[b, :, 0, :]
        invalid_mask = (indices_b < 0) | (indices_b >= s_kv)
        safe_indices = indices_b.masked_fill(invalid_mask, 0)
        
        # Gather KV for this batch: [s_q, topk, d_qk]
        kvs = kv[b, safe_indices, 0, :]
        
        qs = q[b].float()  # [s_q, h_q, d_qk]
        # Compute attention scores: [s_q, h_q, topk]
        attn_score = torch.einsum('shd,std->sht', qs, kvs.float())
        attn_score.masked_fill_(invalid_mask.unsqueeze(1), float("-inf"))
        attn_score = attn_score * sm_scale * LOG2E
        
        max_logits[b] = torch.max(attn_score, dim=-1)[0]
        lse[b] = log2sumexp2(attn_score, dim=-1)
        attn_weights = torch.exp2(attn_score - lse[b].unsqueeze(-1))
        # Output: [s_q, h_q, d_v]
        out[b] = torch.einsum('sht,std->shd', attn_weights, kvs[:, :, :d_v].float()).to(torch.bfloat16)
